...
class ChineseBM25Retriever(BaseRetriever):
    ...
    def __init__(self, docs, k=5, **kwargs):
        ...
        self._docs = docs
        self._k = k
        tokenized_docs = [self._chinese_ngram_tokenize(doc.page_content) for doc in docs]
        self._bm25 = BM25Okapi(tokenized_docs)

    def _chinese_ngram_tokenize(self, text: str) -> list[str]:
        """使用N-gram进行中文分词"""
        import re
        tokens = []
        ...
        tokens.extend(word_pattern.findall(text))
        return tokens

    def _get_relevant_documents(self, query: str, run_manager=None) -> list[Document]:
        query_tokens = self._chinese_ngram_tokenize(query)
...
class VectorStoreService:
    ...
    async def get_bm25_retriever(self):
        ...
        if all_docs:
            return ChineseBM25Retriever(docs=all_docs, k=chroma_config['k'])
        else:
            return None
